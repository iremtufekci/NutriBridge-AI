...
object ClientBottomBarHelper {
    ...
    fun styleTabs(activity: AppCompatActivity, selectedIndex: Int) {
        ...
        val icons = listOf(
            R.id.iconHome, R.id.iconJournal, R.id.iconFood, R.id.iconChef, R.id.iconProfile
        )
        val labels = listOf(
            R.id.labelHome, R.id.labelJournal, R.id.labelFood, R.id.labelChef, R.id.labelProfile
        )
    ...
    fun bind(activity: AppCompatActivity, selectedIndex: Int) {
        ...
        listOf(
            R.id.tabHome, R.id.tabJournal, R.id.tabFood, R.id.tabChef, R.id.tabProfile
...
object ClientTabNav {
    ...
    private const val TAB_FOOD = 2
    private const val TAB_CHEF = 3
    ...
    fun go(activity: AppCompatActivity, index: Int) {
        ...
        if (index in setOf(TAB_JOURNAL, TAB_FOOD, TAB_CHEF)) {
            ...
            val (title, body) = when (index) {
                TAB_JOURNAL -> "Diyet Programım" to
                    "Web’deki günlük program / journal görünümü bu ekrana bağlanacak."
                TAB_FOOD -> "Yemek Analizi" to
                    "Fotoğrafla yemek analizi web ile aynı API’lere buradan bağlanacak."
