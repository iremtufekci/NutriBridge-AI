...
function formatApiError(e: unknown): string {
  if (axios.isAxiosError(e)) {
    ...
    if (s === 403) return "Yetki yok: bu uç nokta yalnızca yönetici (Admin) rolü içindir. Çıkış yapıp yönetici hesabıyla deneyin.";
    if (s === 404) return "Uç nokta bulunamadı (404). Backend’i yeniden başlatıp en son sürümü derlediğinizden emin olun.";
    if (msgFromBody) return msgFromBody;
    if (s) return `HTTP ${s}`;
    if (e.message === "Network Error") {
      return "Ağ hatası: API’ye ulaşılamıyor. Backend’in çalıştığını (ör. 5231) ve Vite’ın proxy kullandığını kontrol edin.";
    }
...
function timeColor(ms: number) {
  if (ms < 200) return "text-emerald-600 dark:text-[#2ECC71]";
  if (ms < 1000) return "text-amber-600 dark:text-amber-400";
  return "text-rose-600 dark:text-rose-400";
}

function errToneClass(t: string) {
  if (t === "high") return "border-l-4 border-rose-500 bg-rose-50/50 dark:bg-rose-950/20";
  if (t === "medium") return "border-l-4 border-amber-500 bg-amber-50/50 dark:bg-amber-950/20";
  return "border-l-4 border-teal-500 bg-teal-50/30 dark:bg-teal-950/15";
}
    ...
    return (
      <SidebarLayout userRole="admin" userName={name}>
        <div className="p-6 max-w-lg space-y-3">
          <p className="text-rose-600 dark:text-rose-400 text-sm leading-relaxed">{err}</p>
          <button
            type="button"
            onClick={() => void load()}
            className="text-sm font-semibold text-emerald-600 dark:text-[#2ECC71] hover:underline"
          >
            Tekrar dene
          </button>
        </div>
      ...
      <div className="min-h-full bg-slate-100/80 dark:bg-slate-950 px-3 sm:px-6 py-4 sm:py-6 pb-24 md:pb-8">
        <div className="mx-auto max-w-7xl space-y-5 sm:space-y-6">
          <div>
            ...
            <p className="text-slate-500 dark:text-[#9CA3AF] text-sm mt-0.5">
              Teknik metrikler, API performansı ve güvenlik (son {data?.dataWindowHours ?? 24} saat, canlı)
            </p>
            {data?.dataNote && <p className="text-xs text-slate-500 dark:text-[#9CA3AF] mt-1">{data.dataNote}</p>}
            {data?.generatedAtUtc && (
          ...
          <div className="bg-white dark:bg-[#1F2937] rounded-2xl border border-slate-200/80 dark:border-slate-700 p-4 sm:p-5 shadow-sm">
            <div className="flex items-center gap-2 mb-3">
              <div className="p-1.5 rounded-lg bg-emerald-100 dark:bg-emerald-900/30">
                <Terminal className="w-4 h-4 text-emerald-600" />
              </div>
              <div>
                <h2 className="font-bold text-slate-900 dark:text-white">API uç noktası performansı</h2>
              ...
              <table className="w-full min-w-[640px] text-sm">
                <thead>
                  <tr className="text-left text-slate-500 border-b border-slate-200 dark:border-slate-700">
                    ...
                    <th className="py-2 pr-2 font-medium">Ort. süre</th>
                    <th className="py-2 pr-2 font-medium">Hata</th>
                    <th className="py-2 font-medium">2xx oranı</th>
                  </tr>
                </thead>
                <tbody>
                  {data?.endpointPerformance.map((row) => (
                    <tr key={row.endpoint} className="border-b border-slate-100 dark:border-slate-800">
                      <td className="py-2.5 pr-2">
                        <code className="text-xs bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-md">{row.endpoint}</code>
                      </td>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <CardChart title="HTTP trafik ayrışımı (GET vs diğer)" subtitle="Sorgu türü dağılımı" icon={Database}>
              <div className="h-64 w-full min-w-0">
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={data?.databaseHourly} margin={{ top: 6, right: 6, left: 0, bottom: 0 }}>
                    ...
                    <Legend />
                    <Bar dataKey="reads" name="GET" fill="#14B8A6" radius={[3, 3, 0, 0]} />
                    <Bar dataKey="writes" name="Diğer" fill="#0D9488" radius={[3, 3, 0, 0]} />
                    <Line type="monotone" dataKey="slowQueries" name=">500ms" stroke="#F97316" dot={false} />
                  </ComposedChart>
                </ResponsiveContainer>
              </div>
            </CardChart>
            <CardChart title="Yanıt dağılımı" subtitle="2xx vs hata" icon={Activity}>
              <div className="h-64 w-full min-w-0">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={data?.cacheHourly} margin={{ top: 6, right: 6, left: 0, bottom: 0 }}>
                    ...
                    <Legend />
                    <Bar dataKey="hits" name="2xx" fill="#22C55E" radius={[2, 2, 0, 0]} />
                    <Bar dataKey="misses" name="Diğer" fill="#EF4444" radius={[2, 2, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </CardChart>
          ...
          <div className="bg-white dark:bg-[#1F2937] rounded-2xl border border-slate-200/80 dark:border-slate-700 p-4 sm:p-5 shadow-sm">
            <div className="flex items-center justify-between gap-2 mb-2">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h2 className="font-bold text-slate-900 dark:text-white">Tahmini ağ yükü</h2>
          ...
          <div className="bg-white dark:bg-[#1F2937] rounded-2xl border border-slate-200/80 dark:border-slate-700 p-4 sm:p-5 shadow-sm">
            <div className="flex items-center gap-2 mb-3">
              ...
              <div>
                <h2 className="font-bold text-slate-900 dark:text-white">Sistem kaynakları (anlık)</h2>
                <p className="text-xs text-slate-500">API işlemi (CPU, RAM) ve disk</p>
              </div>
            </div>
            <p className="text-xs text-slate-500 mb-3">{r.networkNote}</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
              <ResourceTile label="CPU" value={`${r.cpuPercent}%`} refText="/ 100%" barClass="bg-emerald-500" pct={r.cpuPercent} showBar />
              <ResourceTile
                label="Bellek"
              ...
              <ResourceTile
                ...
                showBar
              />
              <div className="rounded-xl border border-slate-200 dark:border-slate-700 p-3 bg-slate-50/80 dark:bg-slate-800/30">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="bg-white dark:bg-[#1F2937] rounded-2xl border border-slate-200/80 dark:border-slate-700 p-4 sm:p-5 shadow-sm">
              <div className="flex items-center gap-2 mb-3">
                ...
                <div>
                  <h2 className="font-bold text-slate-900 dark:text-white">Hata logları</h2>
              ...
              <ul className="space-y-2 max-h-80 overflow-y-auto pr-1">
                {data?.errorLogs.length ? (
                  data.errorLogs.map((e, i) => (
                    <li key={i} className="rounded-xl border border-slate-200 dark:border-slate-600 p-3 text-sm">
                      <div className="flex flex-wrap items-center gap-2 text-xs">
                        <span className="font-mono text-rose-600 bg-rose-100 dark:bg-rose-900/30 px-1.5 py-0.5 rounded">
                          {e.statusCode}
                        </span>
                        <span className="text-slate-500">{e.time}</span>
                        {e.count > 1 && (
                          <span className="ml-auto text-rose-600 bg-rose-50 dark:bg-rose-950/20 px-1.5 rounded">x{e.count}</span>
                        )}
                      </div>
                      <code className="block text-xs text-slate-600 dark:text-slate-300 mt-1 font-mono">{e.endpoint}</code>
            ...
            <div className="bg-white dark:bg-[#1F2937] rounded-2xl border border-slate-200/80 dark:border-slate-700 p-4 sm:p-5 shadow-sm">
              <div className="flex items-center gap-2 mb-3">
                ...
                <div>
                  <h2 className="font-bold text-slate-900 dark:text-white">Güvenlik olayları</h2>
                    ...
                    <li key={i} className={`rounded-xl p-3 text-sm ${errToneClass(e.tone)}`}>
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          ...
                          <p className="font-semibold text-slate-900 dark:text-slate-100 mt-0.5">{e.name}</p>
                        </div>
                        <span className="text-xs text-slate-500 shrink-0">{e.time}</span>
                      </div>
                      <p className="text-xs text-slate-600 dark:text-[#9CA3AF] font-mono mt-1">{e.obfuscatedSource}</p>
...
function Kpi({
  ...
  return (
    <div className="bg-white dark:bg-[#1F2937] rounded-2xl border border-slate-200/80 dark:border-slate-700 p-4 shadow-sm">
      <div className="flex items-start justify-between gap-2">
        <div>
